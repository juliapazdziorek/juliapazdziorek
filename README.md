# Hi, I'm Jula

<p align="center">
  <img src="https://streak-stats.demolab.com?user=juliapazdziorek&theme=catppuccin_mocha" alt="Streak" />
</p>

## About me
A third-year student at the Polish-Japanese Academy of Information Technology in Warsaw. I'm most passionate about **game design and game dev**, and alongside that I'm growing into **artificial intelligence** and **data science**.

### Currently working on
I'm working on two engineering theses (BEng):
- A team project on the game dev specialization — an **isometric game about post-apocalyptic mathematics**
- A **parametrized poker agent** built as a **Unity package**, combining AI with game dev

### Skills & highlights
- Building games in **Unity** (C#) and my own projects in **Java**
- Working with **.NET** and **Entity Framework** on C# projects
- Exploring **AI** and **data science** in Python
- Developing my skills through university courses and team projects
- Most proud of 2nd place at **PJATK Game Jam v8 "Nostalgia Trip"** (24h, November 2025) with the XR_Games team

## Tech
<p align="center">
  <img width="50px" src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/unity/unity-original.svg">
  <img width="50px" src="https://github.com/devicons/devicon/blob/v2.15.1/icons/csharp/csharp-original.svg">
  <img width="50px" src="https://github.com/devicons/devicon/blob/v2.15.1/icons/dotnetcore/dotnetcore-original.svg">
  <img width="50px" src="https://github.com/devicons/devicon/blob/v2.15.1/icons/java/java-original.svg">
  <img width="50px" src="https://github.com/devicons/devicon/blob/v2.15.1/icons/python/python-original.svg">
  <img width="50px" src="https://github.com/devicons/devicon/blob/v2.15.1/icons/git/git-original.svg">
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=juliapazdziorek&layout=compact&theme=catppuccin_mocha" alt="Top Langs" />
</p>

## Featured projects 

### Unity projects
- **[Banana's Era Shift](https://github.com/juliapazdziorek/Nostalgia-Trip-Game-Jam-2025)** — a game from PJATK Game Jam v8, 2nd place, built in 24h with the XR_Games team
- **[Pirate Cove](https://github.com/juliapazdziorek/Pirate-Cove)** — my first Unity project, built over 6 weeks with a team for a PJATK course

### Game Dev project in Java
- **[Idle Cat Farm](https://github.com/juliapazdziorek/Idle-Cat-Farm)** — a cozy indie game in Java where you build and manage a farm run by adorable cat farmers

### Data science and Artificial Intelligence courses from PJATK
- **[Artificial Intelligence Tools](https://github.com/juliapazdziorek/Artificial-Intelligence-Tools)** — a collection of mini-projects from an AI tools course

### Puzzle solutions
- **[Advent of Code](https://github.com/juliapazdziorek/Advent-of-Code)** — my solutions to the Advent of Code puzzles
